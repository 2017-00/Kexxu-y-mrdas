...
read_tobii_eye_data <- function(path) {
  ...
  puntos <- data.frame(
    x = c(807, 984, 1177, 831, 955, 1171, 803, 989, 1186),
    y = c(1080-415, 1080-483, 1080-439, 1080-624, 1080-600, 1080-614, 1080-724, 1080-724, 1080-688)
  )
  ...
  for (i in seq_along(lines)) {
    ...
    if (js$type == "gaze" && !is.null(js$data$eyeright$pupildiameter)) {
      ...
      dif_x <- (x - puntos$x)*(1920/726.5) 
      dif_y <- (y - puntos$y)*(1080/402.5)
      p_x <- puntos$x
      p_y <- puntos$y
...
write.csv(locs_df, file = "/home/luiszamora/Documentos/Eyetracker/tobii/emma/emaexp2-2.csv", row.names = FALSE)

# Densidad de puntos
ggplot(locs_df, aes(x = x, y = y)) +
  geom_point(aes(x = 807, y = 1080 - 415), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 984, y = 1080 - 483), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 1177, y = 1080 - 439), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 831, y = 1080 - 624), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 955, y = 1080 - 600), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 1171, y = 1080 - 614), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 803, y = 1080 - 724), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 989, y = 1080 - 724), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 1186, y = 1080 - 688), color = "black", shape = 8, size = 4) +
