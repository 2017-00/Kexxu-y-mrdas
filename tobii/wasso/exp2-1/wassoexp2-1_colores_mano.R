...
read_tobii_eye_data <- function(path) {
  ...
  puntos <- data.frame(
    x = c(820, 945, 1060, 831, 943, 1072, 830, 950, 1071),
    y = c(1080-417, 1080-407, 1080-405, 1080-491, 1080-518, 1080-484, 1080-608, 1080-586, 1080-583)
  )
  ...
  for (i in seq_along(lines)) {
    ...
    if (js$type == "gaze" && !is.null(js$data$eyeright$pupildiameter)) {
      ...
      dif_x <- (x - puntos$x)*(1920/1038.5) 
      dif_y <- (y - puntos$y)*(1080/578)
      p_x <- puntos$x
      p_y <- puntos$y
...
write.csv(locs_df, file = "/home/luiszamora/Documentos/Eyetracker/tobii/wasso/wassoexp2-1.csv", row.names = FALSE)

# Densidad de puntos
ggplot(locs_df, aes(x = x, y = y)) +
  geom_point(aes(x = 820, y = 1080 - 417), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 945, y = 1080 - 407), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 1060, y = 1080 - 405), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 831, y = 1080 - 491), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 943, y = 1080 - 518), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 1072, y = 1080 - 484), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 830, y = 1080 - 608), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 950, y = 1080 - 586), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 1071, y = 1080 - 583), color = "black", shape = 8, size = 4) +
