...
read_tobii_eye_data <- function(path) {
  ...
  for (i in seq_along(lines)) {
    ...
    if (js$type == "gaze" && !is.null(js$data$eyeright$pupildiameter)) {
      ...
      dif_x <- (x - puntos$x)*(1920/885) 
      dif_y <- (y - puntos$y)*(1080/489.5)
      p_x <- puntos$x
      p_y <- puntos$y
...
write.csv(locs_df, file = "/home/luiszamora/Documentos/Eyetracker/tobii/wasso/exp1/wasso1.csv", row.names = FALSE)

# Densidad de puntos
ggplot(locs_df, aes(x = x, y = y)) +
  geom_point(aes(x = 947, y = 1080 - 520), color = "black", shape = 8, size = 4) +
  geom_point(aes(x = 1189, y = 1080 - 501), color = "black", shape = 8, size = 4) +
