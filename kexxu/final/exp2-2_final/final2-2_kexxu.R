...
read_kexxu_eye_data <- function(path) {
  ...
  locs <- list()
  #Extraemos las variables del archivo 
  for (i in 1:nrow(lines)) {
    ...
    #Se hace esto para que todos los experimentos tengan los mismos puntos de ref
    #Para que se vea bonito y entendible pues, estos puntos los calculamos transformando
    #los grados de visiòn(distancias entre puntos definidas) a pixeles
    if (group == "magenta"){
      x_ref <- 960
      y_ref <- 540
    }
    ...
    #Los nuevos resultados son entonces las diferencias de distancias del csv mas los
    #nuevos puntos de referencia
    x_dif <- x_ref + line$dif_x
    y_dif <- y_ref + line$dif_y
    #Para plotear enciama de los tobii, hacemos que todos los puntos de kexxu sean rojos
    #Dependiendo de lo que necitemos, dejamos comentada o no esta linea
    group <- "red"
...
#write.csv(locs_df, file = "/home/luis-ignacio-zamora/Documents/mudanza/Documentos/Eyetracker/PlotsProba/Kexxu/exp2-2/final2-2_kexxu_csv.csv", row.names = FALSE)
