...
read_kexxu_eye_data <- function(path) {
  ...
  for (i in 1:nrow(lines)) {
    ...
    if (length(locs) > 0) {
      ...
      if ((last_loc$grupo == "purple") && (group == "orange") && (x_dif - last_loc$x > 10)) {
        locs[[length(locs) + 1]] <- list(grupo = group, x = x_dif, y = y_dif, dif = dif)
      }
      else if ((last_loc$grupo == "orange") && (group == "purple") && (last_loc$x - x_dif > 10)) {
        locs[[length(locs) + 1]] <- list(grupo = group, x = x_dif, y = y_dif, dif= dif)
      }
     else if ((last_loc$grupo == "orange") && (group == "orange") && (last_loc$x < x_dif) && ((y_dif-last_loc$y)^2 < 3)) {
        locs[[length(locs) + 1]] <- list(grupo = group, x = x_dif, y = y_dif, dif= dif)
      }
      else if ((last_loc$grupo == "purple") && (group == "purple") && (last_loc$x > x_dif) && ((last_loc$y-y_dif)^2 < 3)) {
        locs[[length(locs) + 1]] <- list(grupo = group, x = x_dif, y = y_dif, dif= dif)
      } 
    }
    else {
...
# Verificar y asegurar que las columnas no sean listas
locs_df <- locs_df %>%
...
ggplot(locs_df, aes(x = x, y = y)) + #475819263
  geom_point(aes(group = 1), color = "blue", size = .3) +
