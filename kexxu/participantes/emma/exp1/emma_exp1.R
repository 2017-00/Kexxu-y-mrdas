...
read_kexxu_eye_data <- function(path) {
  ...
  for (i in seq_along(lines)) {
    ...
    if (grepl("/eyetracking$", parts[1])) {
      ...
      dif_x <- (x - puntos$x)*(1920/403) 
      dif_y <- (y - puntos$y)*(1080/223)
      p_x <- puntos$x
      p_y <- puntos$y
...
#write.csv(locs_df, file = "/home/luiszamora/Documentos/Eyetracker/kexxu/final/exp1_final/emma1.csv", row.names = FALSE)

# Graficar
ggplot(locs_df, aes(x = x, y = y)) + #, color = "blue"
  geom_point(aes(color = group), alpha = 1) +
  geom_path(aes(group = 1), size = .5) +
  theme_minimal() +
  geom_point(aes(x = 621, y = 720-279), color = "black", shape = 3, size = 1.5) +
  geom_point(aes(x = 739, y = 720-278), color = "black", shape = 16, size = 1.5) +
