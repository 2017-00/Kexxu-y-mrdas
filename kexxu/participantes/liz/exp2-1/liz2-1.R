...
read_kexxu_eye_data <- function(path) {
  ...
  puntos <- data.frame(
    x = c(572, 528, 625, 520, 617, 571, 561, 517, 609),
    y = c(720-259, 720-300, 720-289, 720-227, 720-221, 720-300, 720-229, 720-271, 720-264)
  )
  ...
  for (i in seq_along(lines)) {
    ...
    if (grepl("/eyetracking$", parts[1])) {
      ...
      dif_x <- (x - puntos$x)*(1920/400) 
      dif_y <- (y - puntos$y)*(1080/222)
      p_x <- puntos$x
      p_y <- puntos$y
...
write.csv(locs_df, file = "/home/luiszamora/Documentos/Eyetracker/kexxu/final/exp2-1_final_2/liz2-1.csv", row.names = FALSE)
