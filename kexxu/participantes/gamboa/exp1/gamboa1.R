...
read_kexxu_eye_data <- function(path) {
  ...
  for (i in seq_along(lines)) {
    ...
    if (grepl("/eyetracking$", parts[1])) {
      ...
      dif_x <- (x - puntos$x)*(1920/410) 
      dif_y <- (y - puntos$y)*(1080/227)
      p_x <- puntos$x
      p_y <- puntos$y
...
write.csv(locs_df, file = "/home/luiszamora/Documentos/Eyetracker/kexxu/gamboa/exp1/gamboa1.csv", row.names = FALSE)
...
ggplot(locs_df, aes(x = x, y = y)) +
 # geom_point(aes(color = group), alpha = 1) +
  ...
  theme_minimal() +
  geom_point(aes(x = 586, y = 720-298), color = "black", shape = 3, size = 1.5) +
  geom_point(aes(x = 710, y = 720-296), color = "black", shape = 16, size = 1.5) +
